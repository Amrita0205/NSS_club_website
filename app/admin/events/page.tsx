...
export default function AdminEventsPage() {
  ...
  const fetchEvents = async () => {
    try {
      ...
      const res = await axios.get(
        "http://localhost:5000/api/event/all?limit=100&order=desc&sortBy=date"
      );
      if (res.data?.success) {
        ...
        const upcoming = allEvents.filter(
          (event: EventType) => new Date(event.date) > now && !event.isCompleted
        );
        const completed = allEvents.filter(
          (event: EventType) => event.isCompleted || new Date(event.date) <= now
        );
  ...
  const markEventAsCompleted = async (eventId: string) => {
    ...
    try {
      const res = await axios.patch(
        `http://localhost:5000/api/event/${eventId}/complete`,
        {},
        {
      ...
      <main className="flex-1 px-6 py-8">
        ...
        <div className="max-w-7xl mx-auto mb-8">
          ...
          <p className="text-gray-400">
            Create events, view attendance and mark completion
          </p>
        </div>
  ...
  className="bg-gray-800/80 rounded-2xl p-8 border border-gray-700/40 shadow-lg space-y-6"
>
  <h4 className="text-lg font-semibold text-white mb-4">Create New Event</h4>
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
    <div>
      ...
      <input
        ...
        className="w-full bg-gray-700/60 border border-gray-600/60 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-purple-500"
      />
    </div>
  ...
  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div>
      ...
      <input
        ...
        max="24"
        value={givenHours}
        onChange={(e) => setGivenHours(Number(e.target.value))}
        required
        className="w-full bg-gray-700/60 border border-gray-600/60 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-purple-500"
      />
    </div>
  ...
  <div className="space-y-4">
    ...
    <div>
      ...
      <input
        ...
        placeholder="e.g., ID, Waiver Form"
        className="w-full bg-gray-700/60 border border-gray-600/60 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-purple-500"
      />
    </div>
  </div>
  ...
  <div className="flex items-center gap-4">
    <button
      type="submit"
      disabled={submitting}
      className="px-6 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-500 transition-all disabled:opacity-50"
    >
    ...
    {message && <span className="text-sm text-gray-300">{message}</span>}
        ...
        <div className="max-w-7xl mx-auto mt-10">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              onClick={fetchEvents}
              className="px-3 py-2 bg-gray-800/70 rounded-lg border border-gray-700/60 hover:bg-gray-700/60 transition-all"
            >
